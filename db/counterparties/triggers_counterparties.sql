...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.transactions_borrow WHERE counterparty_id = OLD.id)
     OR EXISTS (SELECT 1 FROM public.transactions_lend WHERE counterparty_id = OLD.id) THEN
