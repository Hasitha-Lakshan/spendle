...
-- =========================================
-- 03. Function: prevent_duplicate_exchange_rate
-- =========================================
-- Purpose:
--   Prevents users from creating duplicate or invalid exchange rate records
--   in the exchange_rates table. Ensures each user has only one active
--   (non-deleted) rate per unique currency pair.
--   Intended to be used as a BEFORE INSERT or BEFORE UPDATE trigger.
--
-- Triggered Events:
--   INSERT, UPDATE on exchange_rates
--
-- Behavior:
--   - Raises an exception if the 'from_currency' and 'to_currency'
--     values are identical, since self-referential rates are invalid.
--   - Checks for an existing active row (deleted_at IS NULL) with the same
--     user_id, from_currency, and to_currency combination.
--   - Excludes the current record ID during updates to allow valid edits.
--   - Raises an exception if a duplicate active exchange rate already exists.
--
-- Notes:
--   - Ensures logical uniqueness at the application level, complementing
--     database constraints.
--   - Helps maintain consistent and conflict-free currency rate data.
--   - Returns NEW to allow the operation to proceed when validation passes.
-- =========================================
...
DECLARE
    v_count INT;
BEGIN
    -- Prevent same currency for from and to
    IF NEW.from_currency = NEW.to_currency THEN
        RAISE EXCEPTION 'from_currency and to_currency cannot be the same: % → %', NEW.from_currency, NEW.to_currency;
    END IF;

    -- Check for existing row with same user_id, from_currency, to_currency
    ...
    WHERE user_id = NEW.user_id
      ...
      AND id <> COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'); -- exclude self for updates

    IF v_count > 0 THEN
        RAISE EXCEPTION 'Duplicate exchange rate for user %: % → %', NEW.user_id, NEW.from_currency, NEW.to_currency;
    END IF;
...
-- Trigger for exchange_rates table
CREATE TRIGGER trg_prevent_duplicate_exchange_rate
BEFORE INSERT OR UPDATE ON exchange_rates
FOR EACH ROW
EXECUTE FUNCTION public.prevent_duplicate_exchange_rate();
